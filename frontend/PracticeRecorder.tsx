...
export function PracticeRecorder({
  ...
  const elapsedTime = startTime ? Math.round((performance.now() - startTime) / 1000) : 0
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
      ...
      {playedNotes.length > 0 && (
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div>
            ...
            <p className="text-xl font-bold text-foreground">{playedNotes.length}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Expected</p>
            <p className="text-xl font-bold text-foreground">{expectedNotes.length}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Accuracy</p>
            <p className={`text-xl font-bold ${accuracy >= 80 ? 'text-green-500' : accuracy >= 60 ? 'text-yellow-500' : 'text-red-500'}`}>
              {accuracy}%
            </p>
          </div>
        </div>
      ...
      {playedNotes.length > 0 && (
        <div className="mt-4">
          <p className="text-sm font-medium text-foreground mb-2">Notes Played:</p>
