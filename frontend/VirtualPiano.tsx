...
const PIANO_NOTES: PianoKey[] = [
  ...
  { note: 'F#3', isBlack: true, frequency: 185.00 },
  { note: 'G3', isBlack: false, frequency: 196.00 },
  { note: 'G#3', isBlack: true, frequency: 207.65 },
  { note: 'A3', isBlack: false, frequency: 220.00 },
    ...
    <div className="bg-card border border-border rounded-2xl p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Use keyboard (Z-M) or click to play</p>
        </div>
        <button
          onClick={handleReset}
      ...
      <div className="relative mb-8 h-48 bg-background rounded-lg p-4 overflow-x-auto">
        <div className="flex gap-0.5 relative" style={{ perspective: '1000px' }}>
          {PIANO_NOTES.map((key, idx) => (
            <div key={key.note} className="relative" style={{ flex: key.isBlack ? '0 0 50%' : '0 0 70px' }}>
              {key.isBlack ? (
                // Black keys
                <button
                ...
                >
                  <div className="flex flex-col items-center justify-end h-full pb-2">
                    <span className="text-xs font-medium text-foreground/40">{key.note}</span>
                  </div>
                </button>
              )}
            </div>
      ...
      <div className="flex items-center gap-4">
        ...
        />
        <span className="text-sm text-muted-foreground w-8 text-right">{Math.round(volume * 100)}%</span>
      </div>
